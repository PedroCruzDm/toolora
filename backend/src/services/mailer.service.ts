...
const buildPasswordResetMessage = ({ name, code, resetUrl, expiresInMinutes }: PasswordResetEmailInput) => ({
  ...
  text: [`Olá, ${name}.`, '', 'Recebemos uma solicitação para redefinir sua senha na Toolora.',
    `Seu código de verificação é: ${code}`, `Este código expira em ${expiresInMinutes} minutos.`, '',
    `Abra este link para concluir a redefinição: ${resetUrl}`, '', 'Se você não solicitou isso, pode ignorar este email.',
  ].join('\n'),
